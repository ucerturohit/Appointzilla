				 ...
				 *************************/
					...
					else
					{
						...
						if($AllAppointmentsData)
						{
							...
							$EventTableName = $wpdb->prefix."ap_events";
							$AllEventts_sql = "SELECT `start_time`, `end_time` FROM `$EventTableName` WHERE date('$AppointmentDate') between `start_date` AND `end_date` AND `allday` = '0' AND `repeat` != 'W' AND `repeat` != 'M'";
							...
							if($AllEventsData)
							{
								foreach($AllEventsData as $Event)
								{
									...
									for( $i = $Current_Re_Start_Date; $i <= $Current_Re_End_Date; $i += (60 * 60 * 24 * 7)) //make weekly dates
									{
							...
							if($AllEventsData)
							{
								foreach($AllEventsData as $Event)
								{
									...
									for( $i = $Current_Re_Start_Date; $i <= $Current_Re_End_Date; $i += (60 * 60 * 24 * 31)) //make weekly dates
									{
										$AllEventMonthlyDates[] = date('Y-m-d', $i);
									}
